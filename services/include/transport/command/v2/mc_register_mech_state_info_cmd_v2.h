 ...
 */
#ifndef MC_REGISTER_MECH_STATE_INFO_CMD_V2_H
#define MC_REGISTER_MECH_STATE_INFO_CMD_V2_H
...
class RegisterMechStateInfoCmdV2 : public RegisterMechStateInfoCmd {
public:
    ...
    static constexpr uint16_t RPT_SIZE = 0; // TLV

    explicit RegisterMechStateInfoCmdV2();
    ~RegisterMechStateInfoCmdV2() override = default;
    ...
    uint8_t GetEventType() const override;
    const MechStateInfo& GetInfo() const override;
    const RotationAxesStatus& GetRotationAxesStatus() const override;
    uint8_t GetResult() const override;

private:
    void HandleAxesStatus(uint8_t value);
    void HandleElectricityInfo(uint8_t value);
};
} // namespace MechBodyController
} // namespace OHOS

#endif // MC_REGISTER_MECH_STATE_INFO_CMD_V2_H
