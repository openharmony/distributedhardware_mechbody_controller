...
int32_t MechbodyAdapterUtils::InitTrackingCore()
{
    ...
    dlerror();
    HILOGI("end load mechbody adapter.");
    int32_t ret = ERR_OK;
    do {
        ...
        if (dlsymError != nullptr) {
            HILOGE("Failed to get init function: %{public}s", dlsymError);
        ...
        dlsymError = dlerror();
        if (dlsymError != nullptr) {
            HILOGE("Failed to get RunTrackingCore function: %{public}s", dlsymError);
            ret = LOAD_MECH_ADAPTER_FUNCTION_ERROR;
            break;
        }
        
        HILOGI("Init remote mech adapter function success.");
