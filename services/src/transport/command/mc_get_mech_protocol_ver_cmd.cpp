...
namespace {
    const std::string TAG = "GetMechProtocolVerCmd";
}

GetMechProtocolVerCmd::GetMechProtocolVerCmd()
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
    reqSize_ = REQ_SIZE;
...
std::shared_ptr<MechDataBuffer> GetMechProtocolVerCmd::Marshal() const
{
    HILOGI("start.");
...
void GetMechProtocolVerCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    if (data == nullptr) {
    ...
    CHECK_ERR_RETURN(data->ReadUint8(offset, highProtocolVer_), "read protocolVer");
    offset++;

    CHECK_ERR_RETURN(data->ReadUint8(offset, lowProtocolVer_), "read protocolVer");
    offset++;
...
uint8_t GetMechProtocolVerCmd::GetParams() const
{
    return lowProtocolVer_;
}

uint8_t GetMechProtocolVerCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
