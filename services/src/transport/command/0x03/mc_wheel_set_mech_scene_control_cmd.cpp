...
namespace {
    const std::string TAG = "WheelSetMechSceneControlCmd";
}

WheelSetMechSceneControlCmd::WheelSetMechSceneControlCmd(ActionType actionType)
...
std::shared_ptr<MechDataBuffer> WheelSetMechSceneControlCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint32(static_cast<uint32_t>(actionType_)), nullptr, "append actionType_");
    HILOGI("end.");
    return buffer;
}

void WheelSetMechSceneControlCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
ActionType WheelSetMechSceneControlCmd::GetActionType() const
{
    return actionType_;
}

uint8_t WheelSetMechSceneControlCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
