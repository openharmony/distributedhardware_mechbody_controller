...
namespace {
    const std::string TAG = "WheelSetMechRotationTraceCmd";
    constexpr size_t CMD_MAX_POINT_SIZE = 16;
    constexpr uint16_t CMD_SIZE_PER_ROTATE_POINT = 8;
    constexpr uint8_t CMD_SET_ROTATE_CONTROL_RALEVANT = 0b00000000;
}

WheelSetMechRotationTraceCmd::WheelSetMechRotationTraceCmd(uint16_t taskId, const std::vector<RotateParam>& params)
...
std::shared_ptr<MechDataBuffer> WheelSetMechRotationTraceCmd::Marshal() const
{
    HILOGD("start.");
    ...
    for (uint8_t i = 0; i < pointNum; i++) {
        ...
        CHECK_ERR_RETURN_VALUE(buffer->AppendInt16(params_[i].forwardSpeed), nullptr, "append forwardSpeed");
        CHECK_ERR_RETURN_VALUE(buffer->AppendFloat(params_[i].turningSpeed), nullptr, "append turningSpeed");
    }
    HILOGD("end.");
    return buffer;
}

void WheelSetMechRotationTraceCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    ...
    HILOGI("taskId: %{public}u", taskId_);
...
const std::vector<RotateParam>& WheelSetMechRotationTraceCmd::GetParams() const
{
    return params_;
}

uint8_t WheelSetMechRotationTraceCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
