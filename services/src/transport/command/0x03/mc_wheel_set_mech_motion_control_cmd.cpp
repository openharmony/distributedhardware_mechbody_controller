...
namespace {
    const std::string TAG = "WheelSetMechMotionControlCmd";
}

WheelSetMechMotionControlCmd::WheelSetMechMotionControlCmd(ControlCommand action)
    : action_(action)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> WheelSetMechMotionControlCmd::Marshal() const
{
    HILOGI("start.");
...
void WheelSetMechMotionControlCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
ControlCommand WheelSetMechMotionControlCmd::GetAction() const
{
    return action_;
}

uint8_t WheelSetMechMotionControlCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
