...
namespace {
    const std::string TAG = "SetMechDevicePairingInfoCmd";
}

SetMechDevicePairingInfoCmd::SetMechDevicePairingInfoCmd(uint32_t deviceIdentifier)
...
std::shared_ptr<MechDataBuffer> SetMechDevicePairingInfoCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint32(deviceIdentifier_), nullptr, "append deviceIdentifier_");

    HILOGI("end.");
    return buffer;
}

void SetMechDevicePairingInfoCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    if (data == nullptr) {
...
uint8_t SetMechDevicePairingInfoCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
