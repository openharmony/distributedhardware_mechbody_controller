...
namespace {
    const std::string TAG = "SetMechMotionControlCmd";
}

SetMechMotionControlCmd::SetMechMotionControlCmd(ControlCommand action)
    : action_(action)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> SetMechMotionControlCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(static_cast<uint8_t>(action_)), nullptr, "append action");

    HILOGI("end.");
    return buffer;
}

void SetMechMotionControlCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
ControlCommand SetMechMotionControlCmd::GetAction() const
{
    return action_;
}

uint8_t SetMechMotionControlCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
