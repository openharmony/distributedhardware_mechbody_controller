...
namespace {
    const std::string TAG = "NormalSetMechProtocolVerCmd";
}

NormalSetMechProtocolVerCmd::NormalSetMechProtocolVerCmd(uint8_t protocolVer)
...
std::shared_ptr<MechDataBuffer> NormalSetMechProtocolVerCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(protocolVer_), nullptr, "append protocolVer");

    HILOGI("end.");
    return buffer;
}

void NormalSetMechProtocolVerCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    if (responseCb_) {
