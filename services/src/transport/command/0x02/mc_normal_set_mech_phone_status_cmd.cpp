...
namespace {
    const std::string TAG = "NormalSetMechPhoneStatusCmd";
}
 
NormalSetMechPhoneStatusCmd::NormalSetMechPhoneStatusCmd(const ScreenInfoParams& screenInfo)
...
std::shared_ptr<MechDataBuffer> NormalSetMechPhoneStatusCmd::Marshal() const
{
    HILOGI("start.");
    ...
    uint8_t screenInfoByte = 0;
    screenInfoByte |= screenInfo_.isPortrait << BIT_0;
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(screenInfoByte), nullptr, "append controlByte");
 
    HILOGI("end.");
    return buffer;
}
 
void NormalSetMechPhoneStatusCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
uint8_t NormalSetMechPhoneStatusCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
