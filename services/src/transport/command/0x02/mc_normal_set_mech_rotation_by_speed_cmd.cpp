...
namespace {
    const std::string TAG = "NormalSetMechRotationBySpeedCmd";
    constexpr uint8_t NEG_LIMITED = 2;
}

NormalSetMechRotationBySpeedCmd::NormalSetMechRotationBySpeedCmd(const RotateBySpeedParam& params)
    : params_(params)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> NormalSetMechRotationBySpeedCmd::Marshal() const
{
    HILOGW("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(BIT_0), nullptr, "append Predictive limit");
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint16(static_cast<int16_t>(params_.duration)), nullptr, "append duration");
...
void NormalSetMechRotationBySpeedCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
const RotateBySpeedParam& NormalSetMechRotationBySpeedCmd::GetParams() const
{
    return params_;
}

const RotationAxesStatus& NormalSetMechRotationBySpeedCmd::GetRotationAxesStatus() const
{
    return status_;
}

uint8_t NormalSetMechRotationBySpeedCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
