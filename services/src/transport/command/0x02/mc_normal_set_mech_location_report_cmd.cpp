...
namespace {
    const std::string TAG = "NormalSetMechLocationReportCmd";
}

NormalSetMechLocationReportCmd::NormalSetMechLocationReportCmd(uint8_t reportSwitch, uint8_t reportFrequency)
    : reportSwitch_(reportSwitch), reportFrequency_(reportFrequency)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
    reqSize_ = REQ_SIZE;
...
std::shared_ptr<MechDataBuffer> NormalSetMechLocationReportCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(reportSwitch_), nullptr, "append Reporting switch");
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(reportFrequency_), nullptr, "append Reporting frequency");

    HILOGI("end.");
    return buffer;
}

void NormalSetMechLocationReportCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
uint8_t NormalSetMechLocationReportCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
