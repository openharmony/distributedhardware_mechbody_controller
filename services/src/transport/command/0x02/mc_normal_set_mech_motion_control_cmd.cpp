...
namespace {
    const std::string TAG = "NormalSetMechMotionControlCmd";
}

NormalSetMechMotionControlCmd::NormalSetMechMotionControlCmd(ControlCommand action)
    : action_(action)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> NormalSetMechMotionControlCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(static_cast<uint8_t>(action_)), nullptr, "append Reporting switch");

    HILOGI("end.");
    return buffer;
}

void NormalSetMechMotionControlCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
ControlCommand NormalSetMechMotionControlCmd::GetAction() const
{
    return action_;
}

uint8_t NormalSetMechMotionControlCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
