...
namespace {
    const std::string TAG = "NormalSetMechCameraInfoCmd";
}

NormalSetMechCameraInfoCmd::NormalSetMechCameraInfoCmd(const CameraInfoParams& params)
    : params_(params)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> NormalSetMechCameraInfoCmd::Marshal() const
{
    HILOGI("start.");
...
void NormalSetMechCameraInfoCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
const CameraInfoParams& NormalSetMechCameraInfoCmd::GetParams() const
{
    return params_;
}
} // namespace MechBodyController
