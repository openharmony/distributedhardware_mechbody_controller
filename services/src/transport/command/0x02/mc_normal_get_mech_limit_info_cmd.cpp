...
namespace {
    const std::string TAG = "NormalGetMechLimitInfoCmd";
}

NormalGetMechLimitInfoCmd::NormalGetMechLimitInfoCmd()
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
    reqSize_ = REQ_SIZE;
...
std::shared_ptr<MechDataBuffer> NormalGetMechLimitInfoCmd::Marshal() const
{
    HILOGI("start.");
...
void NormalGetMechLimitInfoCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
const RotateDegreeLimit& NormalGetMechLimitInfoCmd::GetParams() const
{
    return params_;
}

uint8_t NormalGetMechLimitInfoCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
