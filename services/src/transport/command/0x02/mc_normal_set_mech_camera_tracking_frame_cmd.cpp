...
namespace {
    const std::string TAG = "NormalSetMechCameraTrackingFrameCmd";
}

NormalSetMechCameraTrackingFrameCmd::NormalSetMechCameraTrackingFrameCmd(const TrackingFrameParams& params)
    : params_(params)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> NormalSetMechCameraTrackingFrameCmd::Marshal() const
{
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint64(BIT_0), nullptr, "append BIT_0");
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8((params_.cameraType != CameraType::BACK)), nullptr, "append cameraType");
...
void NormalSetMechCameraTrackingFrameCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    if (responseCb_) {
...
const TrackingFrameParams& NormalSetMechCameraTrackingFrameCmd::GetParams() const
{
    return params_;
}
} // namespace MechBodyController
