...
namespace {
    const std::string TAG = "NormalGetMechStateInfoCmd";
}

NormalGetMechStateInfoCmd::NormalGetMechStateInfoCmd()
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
    reqSize_ = REQ_SIZE;
...
std::shared_ptr<MechDataBuffer> NormalGetMechStateInfoCmd::Marshal() const
{
    HILOGI("start.");
...
void NormalGetMechStateInfoCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    HILOGI("start.");
    ...
    params_.attached = (bitResult >> 0) & 1;
    params_.yawDisable = (bitResult >> 1) & 1;
    params_.rollDisable = (bitResult >> 2) & 1;
    params_.pitchDisable = (bitResult >> 3) & 1;
    offset++;
...
DeviceStateInfo NormalGetMechStateInfoCmd::GetParams() const
{
    return params_;
}

uint8_t NormalGetMechStateInfoCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
