...
namespace {
    const std::string TAG = "SetMechCameraTrackingEnableCmd";
}

SetMechCameraTrackingEnableCmd::SetMechCameraTrackingEnableCmd(MechTrackingStatus status)
    : status_(status)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> SetMechCameraTrackingEnableCmd::Marshal() const
{
    HILOGI("start.");
...
void SetMechCameraTrackingEnableCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
...
MechTrackingStatus SetMechCameraTrackingEnableCmd::GetStatus() const
{
    return status_;
}

uint8_t SetMechCameraTrackingEnableCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
