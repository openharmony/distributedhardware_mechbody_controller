...
namespace {
    const std::string TAG = "SetMechDisconnectCmd";
}

SetMechDisconnectCmd::SetMechDisconnectCmd(uint8_t reason)
    : reason_(reason)
{
    cmdSet_ = CMD_SET;
    cmdId_ = CMD_ID;
...
std::shared_ptr<MechDataBuffer> SetMechDisconnectCmd::Marshal() const
{
    HILOGI("start.");
    ...
    CHECK_ERR_RETURN_VALUE(buffer->AppendUint8(reason_), nullptr, "append reason_");

    HILOGI("end.");
    return buffer;
}

void SetMechDisconnectCmd::TriggerResponse(std::shared_ptr<MechDataBuffer> data)
{
    if (data == nullptr) {
...
uint8_t SetMechDisconnectCmd::GetResult() const
{
    return result_;
}
} // namespace MechBodyController
