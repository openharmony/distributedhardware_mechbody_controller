...
#include "mcmotionmanagernineteen_fuzzer.h"
...
void InitMotionManagerWithWheelBase()
{
    if (g_motionManager == nullptr) {
        g_mockAdapter = std::make_shared<MockTransportSendAdapter>();
        ...
        MechInfo testMech;
        testMech.mechId = TEST_MECH_ID;
        testMech.mechType = MechType::WHEEL_BASE;
        testMech.mac = "AA:BB:CC:DD:EE:FF";
        testMech.state = AttachmentState::ATTACHED;
        testMech.deviceIdentifier = 0x00000000;
        MechConnectManager::GetInstance().AddMechInfo(testMech);
...
void FuzzUnRegisterNotifyEventMultipleTimes(FuzzedDataProvider &provider)
{
    InitMotionManager();
...
void FuzzExecuteRotateCommandWithDifferentTaskIds(FuzzedDataProvider &provider)
{
    InitMotionManager();
    RotateParam param;
...
void FuzzSetDevicePairingInfoWithExtremeValues(FuzzedDataProvider &provider)
{
