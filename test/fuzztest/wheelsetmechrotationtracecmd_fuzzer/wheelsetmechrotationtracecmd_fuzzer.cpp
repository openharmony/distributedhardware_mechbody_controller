...
#include "wheelsetmechrotationtracecmd_fuzzer.h"
...
void FuzzConstructorEmpty(FuzzedDataProvider &provider)
{
    ...
    WheelSetMechRotationTraceCmd cmd(taskId, params);
}
...
{
    uint16_t taskId = provider.ConsumeIntegral<uint16_t>();
    size_t pointNum = provider.ConsumeIntegralInRange<size_t>(2, 8);
    std::vector<RotateParam> params;
...
void FuzzConstructorExceedMax(FuzzedDataProvider &provider)
{
    uint16_t taskId = provider.ConsumeIntegral<uint16_t>();
    std::vector<RotateParam> params;
    size_t pointNum = provider.ConsumeIntegralInRange<size_t>(17, 32);
    for (size_t i = 0; i < pointNum; i++) {
        RotateParam param;
        param.duration = provider.ConsumeIntegral<int32_t>();
        param.forwardSpeed = provider.ConsumeIntegral<int16_t>();
...
{
    uint16_t taskId = provider.ConsumeIntegral<uint16_t>();
    size_t pointNum = provider.ConsumeIntegralInRange<size_t>(0, 16);
    std::vector<RotateParam> params;
    for (size_t i = 0; i < pointNum; i++) {
        RotateParam param;
        param.duration = provider.ConsumeIntegral<int32_t>();
...
void FuzzGetParams(FuzzedDataProvider &provider)
{
    ...
    WheelSetMechRotationTraceCmd cmd(taskId, params);
    const std::vector<RotateParam>& resultParams = cmd.GetParams();
    (void)resultParams;
}
