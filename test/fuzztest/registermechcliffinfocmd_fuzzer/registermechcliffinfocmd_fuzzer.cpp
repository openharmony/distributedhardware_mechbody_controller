...
void FuzzUnmarshalWithOneCliff(FuzzedDataProvider &provider)
{
    ...
    size_t totalSize = RPT_SIZE + BIT_OFFSET_2 + BIT_OFFSET_2 + 1;
...
void FuzzUnmarshalWithMultipleCliffs(FuzzedDataProvider &provider)
{
    ...
    uint8_t cliffNums = provider.ConsumeIntegralInRange<uint8_t>(2, 10);
    size_t totalSize = RPT_SIZE + BIT_OFFSET_2 + cliffNums * (BIT_OFFSET_2 + 1);
...
void FuzzFullWorkflow(FuzzedDataProvider &provider)
{
    ...
    if (shouldTestNull) {
        ...
        uint8_t cliffNums = provider.ConsumeIntegralInRange<uint8_t>(0, 10);
        size_t totalSize = RPT_SIZE + BIT_OFFSET_2 + cliffNums * (BIT_OFFSET_2 + 1);
