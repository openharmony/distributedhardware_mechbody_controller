...
#include "normalsetmechrotationbyspeedcmd_fuzzer.h"
...
void FuzzConstructor(FuzzedDataProvider &provider)
{
    ...
    params.duration = provider.ConsumeIntegral<uint16_t>();
    params.speed.yawSpeed = provider.ConsumeFloatingPoint<float>();
    params.speed.rollSpeed = provider.ConsumeFloatingPoint<float>();
    params.speed.pitchSpeed = provider.ConsumeFloatingPoint<float>();

    NormalSetMechRotationBySpeedCmd cmd(params);
}
...
void FuzzGetParams(FuzzedDataProvider &provider)
{
    ...
    NormalSetMechRotationBySpeedCmd cmd(params);
    const RotateBySpeedParam& resultParams = cmd.GetParams();
    (void)resultParams;
}
