...
#include "mcmotionmanagersixteen_fuzzer.h"
...
namespace {
const std::string TAG = "MotionManagerSixteenFuzz";
...
void FuzzMechWheelZoomNotifyMultipleCalls(FuzzedDataProvider &provider)
{
    InitMotionManager();
...
void FuzzConnectAbilityMultipleCalls(FuzzedDataProvider &provider)
{
    InitMotionManager();
    int32_t iterations = provider.ConsumeIntegralInRange<int32_t>(1, 10);
    for (int32_t i = 0; i < iterations; ++i) {
...
void FuzzConnectServiceExtensionMultipleCalls(FuzzedDataProvider &provider)
{
    InitMotionManager();
    int32_t iterations = provider.ConsumeIntegralInRange<int32_t>(1, 10);
    for (int32_t i = 0; i < iterations; ++i) {
...
void FuzzIsAiDispatchServiceInForegroundWithAppList(FuzzedDataProvider &provider)
{
    InitMotionManager();
    ...
    for (int32_t i = 0; i < appCount; ++i) {
        ...
        appData.bundleName = provider.ConsumeRandomLengthString();
        appData.state = provider.ConsumeIntegral<int32_t>();
...
void FuzzIsLimitedWithRandomValues(FuzzedDataProvider &provider)
{
    InitMotionManager();
    int32_t iterations = provider.ConsumeIntegralInRange<int32_t>(1, 100);
    for (int32_t i = 0; i < iterations; ++i) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    FuzzMechWheelZoomNotifyMultipleCalls(provider);
    FuzzConnectAbilityMultipleCalls(provider);
    FuzzConnectServiceExtensionMultipleCalls(provider);
