...
#include "normalgetmechcoordinateinfocmd_fuzzer.h"
...
constexpr uint8_t HEADER_BYTE2 = 0x07;
...
void FuzzConstructor(FuzzedDataProvider &provider)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        NormalGetMechCoordinateInfoCmd cmd;
    }
}
...
void FuzzGetParams(FuzzedDataProvider &provider)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        NormalGetMechCoordinateInfoCmd cmd;
        DeviceCoordinateInfo params = cmd.GetParams();
        (void)params;
    }
}
