...
HWTEST_F(MechConnectManagerTest, GetConnectMechList_001, TestSize.Level3)
{
    ...
    EXPECT_NE(it1, mechInfos.end());
    if (it1 != mechInfos.end()) {
    ...
    EXPECT_NE(it2, mechInfos.end());
    if (it2 != mechInfos.end()) {
 ...
 * @tc.desc: test NotifyMechState with WHEEL_BASE and first connect
 * @tc.type: FUNC
 */
HWTEST_F(MechConnectManagerTest, NotifyMechState_002, TestSize.Level3)
{
    ...
    int32_t addRet = MechConnectManager::GetInstance().AddDeviceChangeListener(listener);
    ASSERT_EQ(addRet, ERR_OK);
    // 通过NotifyMechConnect添加WHEEL_BASE设备（首次连接）
    MechInfo mechInfo;
    mechInfo.mechId = 1;
    ...
    int32_t connectRet = MechConnectManager::GetInstance().NotifyMechConnect(mechInfo);
    ASSERT_EQ(connectRet, ERR_OK);
    // 验证设备已添加且isFirstConnect为true
    MechInfo addedInfo;
    bool getRet = MechConnectManager::GetInstance().GetMechBasicInfo(1, addedInfo);
    ASSERT_TRUE(getRet);
    ASSERT_EQ(addedInfo.isFirstConnect, true);
    // When: 调用NotifyMechState更新状态为DETACHED（isAttached=false）
    bool ret = MechConnectManager::GetInstance().NotifyMechState(1, false);
    // Then: 应该返回成功
    EXPECT_EQ(ret, true);
    // 验证设备状态已更新
    MechInfo resultInfo;
 ...
 * @tc.desc: test NotifyMechState with PORTABLE_GIMBAL and detach state
 * @tc.type: FUNC
 */
HWTEST_F(MechConnectManagerTest, NotifyMechState_003, TestSize.Level3)
{
    ...
    int32_t addRet = MechConnectManager::GetInstance().AddDeviceChangeListener(listener);
    ASSERT_EQ(addRet, ERR_OK);
    // 通过NotifyMechConnect添加PORTABLE_GIMBAL设备
    MechInfo mechInfo;
    mechInfo.mechId = 1;
 ...
 * @tc.desc: test GetMechState with ATTACHED state
 * @tc.type: FUNC
 */
HWTEST_F(MechConnectManagerTest, GetMechState_002, TestSize.Level3)
{
    ...
    bool getRet = MechConnectManager::GetInstance().GetMechBasicInfo(1, addedInfo);
    ASSERT_TRUE(getRet);
    // When: 调用GetMechState获取设备状态
    AttachmentStateMap ret = MechConnectManager::GetInstance().GetMechState(1);
    // Then: 应该返回ATTACHED状态（NotifyMechConnect后设备状态为ATTACHED）
    EXPECT_EQ(ret, AttachmentStateMap::ATTACHED);
    // 清理：移除监听器
    MechConnectManager::GetInstance().RemoveDeviceChangeListener(listener);
 ...
 * @tc.desc: test GetMechState with DETACHED state
 * @tc.type: FUNC
 */
HWTEST_F(MechConnectManagerTest, GetMechState_003, TestSize.Level3)
{
    ...
    bool getRet = MechConnectManager::GetInstance().GetMechBasicInfo(1, addedInfo);
    ASSERT_TRUE(getRet);
    // 通过NotifyMechState将设备状态更新为DETACHED
    bool stateRet = MechConnectManager::GetInstance().NotifyMechState(1, false);
    ASSERT_TRUE(stateRet);
    // When: 调用GetMechState获取设备状态
    AttachmentStateMap ret = MechConnectManager::GetInstance().GetMechState(1);
    // Then: 应该返回DETACHED状态
